...
| 是否独立 Agent | 否 | 否 | 否 |
| 示例 | 同义替换与定位 Skill | `get_user_profile` | `validate_generated_task` |
...
| 第一次主线计划 | 选能力、Skill、难度、提示 | `get_user_profile` | `validate_generated_task` |
| 第一次主线判分 | 分析错因、提出更新 | `submit_profile_update_suggestion` | `grade_objective_answers`、`record_learning_evidence` |
| 机场副线后 | 读取待验证信号 | `get_user_profile` | 写入副线信号的 Workflow 服务 |
| 第二次计划 | 根据画像和副线信号调整计划 | `get_user_profile` | `validate_generated_task` |
